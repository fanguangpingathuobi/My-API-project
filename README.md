...
欢迎使用火币 API！ 你可以使用此 API 获得火币平台的行情数据，进行交易，及管理你的账户。

在文档的右侧是代码，目前我们仅提供针对 `shell` 的代码示例。

你可以通过点击上方菜单来切换不同业务的API，还可通过点击右上方的语言按钮来切换文档语言。
...
如需使用API ，请先登录网页端，完成API key的申请和权限配置，再根据此文档详情进行开发交易。
...
同时，为保证API服务的稳定性，建议使用日本AWS云服务器进行访问。如使用中国大陆境内的客户端服务器，连接的稳定性将难以保证。
