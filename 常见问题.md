...
<a href='python_signature_demo.md'>Python签名&demo </a>   
<a href='java_signature_demo.md'>JAVA签名&demo </a>  
<a href='cpp_signature_demo'>CPP签名&demo </a>  
...
- account-id 是否正确，是否由 GET /v1/account/accounts 接口返回的数据。
- 可能为网络原因，请稍后进行重试。
...
- 是否将AccessKeyId参数带入URL中。
- account-id 是否正确，是由 GET /v1/account/accounts 接口返回的数据。
- POST 请求是否把业务参数也计算进签名。
- GET 请求是否将参数按照 ASCII 码表顺序排序。
...
### Q9: 如何获取杠杆类交易的币种对？
A: 您可以根据/v1/common/symbols接口返回数据中的字段区分。 leverage-ratio > 0 代表支持逐仓杠杆。super-magin-leverage-ratio > 0 代表支持全仓杠杆。具体数值代表所支持杠杆倍数。
...
参考接口文档地址：https://huobiapi.github.io/docs/spot/v1/cn/#apiv2  
...
参考接口文档地址：https://huobiapi.github.io/docs/spot/v1/cn/#apiv2-3 
